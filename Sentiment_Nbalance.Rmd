...
Выбираем только те слова, которые повторяются больше 100 раз
```{r}
...
bing_word_counts %>%
  group_by(sentiment) %>%
  slice_max(n, n = 5, with_ties = FALSE) %>% #with_ties = FALSE
  ungroup() %>%
...
ggsave('nbalance_bing.png', units = 'in', dpi = 400)
```
...
ggsave('nbalance_nrc.png', units = 'in', dpi = 400, scale = 1.2)
```
